...
router.get('/new', function(req, res, next) {
    res.render('ajoutEntreprise', { title: 'Nouvelle Entreprise', erreur: '', css: "index.css", session: req.session });
});
...
router.post('/new', function(req, res, next) {
    ...
    connection.query('SELECT email FROM entreprises WHERE email=?', [values.email], function(err, liste, fie) {
        if (liste.length == 0) {
            ...
            res.render('ajoutEntreprise', { title: 'Nouvelle Entreprise', erreur: 'email déjà utilisé', css: "index.css", session: req.session });
        }
    });
});
